            ...
            <div className="flex min-h-full items-center justify-center p-4">
                ...
                <div className="bg-surface border border-border rounded-2xl p-6 w-full md:w-[480px] flex-shrink-0">
                    ...
                    <form onSubmit={handleSave} className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            >
                                {COLUMNS.map(col => (
                                    <option key={col.id} value={col.id} style={{ color: col.color }}>{col.label}</option>
                                ))}
                            </select>
                        </div>
                        ...
                        {showOptional && (
                            <div className="flex flex-col gap-4">
                                ...
                                {/* AI Match Score — только на мобиле, на десктопе в отдельной панели */}
                                                ...
                                                <div className="bg-bg border border-border rounded-lg p-3 mb-2">
                                                    ...
                                                    <ul className="flex flex-col gap-1.5">
                                                        {aiScore.points.map((point, i) => (
                                                            <li key={i} className="text-xs leading-relaxed" style={{
                                                                color: point.positive ? '#22C55E' : '#EF4444'
                                                            }}>
                                            ...
                                            {isPro ? (
                                                <div className="flex gap-2">
                                                    <select
                                                        ...
                                                        className="px-2 py-2.5 bg-bg border border-border rounded-lg text-text text-xs outline-none cursor-pointer"
                                                    >
                                                ...
                                                <div className="flex items-center justify-between py-2.5 px-3.5 bg-bg border border-border rounded-lg">
                                                    ...
                                                    <a href="/board" className="text-xs font-semibold text-text no-underline">Upgrade →</a>
                                                </div>
                                            )}
                                        </div>
                                    ) : (
                                        <p className="text-xs text-muted border border-dashed border-border rounded-lg p-2 text-center">
                                            Save job description to use AI Match. Add your CV in Settings.
                                        </p>
                                    )}
                                </div>
                ...
                <div className="hidden md:flex flex-col bg-surface border border-border rounded-2xl p-4 w-102 flex-shrink-0">
                    ...
                    <p className="text-xs text-muted mb-3 leading-relaxed">
                        {!application.job_description
                            ? 'Add job description in Optional fields, then save to enable AI analysis.'
                            : !isPro
                                ? 'Upgrade to Pro to unlock AI match scoring.'
                                : 'Compare your resume against this job.'}
                    </p>

                    {application.job_description && isPro && (
                        <>
                            {aiScore && (
                                <div className="mb-3">
                                    <div className="flex items-center justify-between mb-2">
                            ...
                            {aiError && <p className="text-danger text-xs mb-2">{aiError}</p>}

                            <select
                                value={aiLanguage}
                                onChange={e => setAiLanguage(e.target.value)}
                                className="w-full px-2 py-1.5 bg-bg border border-border rounded-lg text-text text-xs outline-none cursor-pointer mb-2"
                            >
                            ...
                            <button
                                ...
                                onClick={handleAnalyzeMatch}
                                disabled={aiLoading}
                                className="w-full py-2 bg-transparent border border-border text-muted rounded-lg text-xs font-semibold hover:border-muted hover:text-text disabled:opacity-50 transition-colors cursor-pointer"
                            >
                                {aiLoading ? 'Analyzing...' : aiScore ? '✨Re-analyze' : 'Analyze match'}
                            </button>
                        </>
                    ...
                    {!isPro && application.job_description && (
                        <a href="/board" className="block text-center py-2 bg-text text-bg rounded-lg text-xs font-semibold no-underline hover:opacity-90 transition-opacity">
                            Upgrade to Pro →
                        </a>
                    )}
                </div>
